...
fn get_delta() -> Delta<TokenCategory, State> {
    ...
    vec![(Initial, Box::new(|c| c.is_whitespace()), Initial, Box::new(action_null)),
         (Initial, Box::new(|c| c == '('), End, build_action(Cat::ParOpen, true)),
         (Initial, Box::new(|c| c == ')'), End, build_action(Cat::ParClose, true)),
         (Initial,
          Box::new(|c| c == '+' || c == '-' || c == '*'),
          TrailingWS,
          build_action(Cat::OpMat, true)),
         (Initial, Box::new(|c| c == '='), TrailingWS, build_action(Cat::OpRel, true)),
         (Initial, Box::new(|c| c == '>' || c == '<'), OpRelComp, build_action(Cat::OpRel, true)),
         (Initial, Box::new(|c| c == '"'), Str, build_action(Cat::Str, true)),
         (Initial, Box::new(|c| c.is_alphabetic()), Id, build_action(Cat::Id, true)),
         (Initial, Box::new(|c| c.is_numeric()), Number, build_action(Cat::Number, true)),
         (Initial, Box::new(|_| true), Error, build_error_action("BAD INIT TOKEN")),

         (TrailingWS, Box::new(|c| c.is_whitespace()), End, Box::new(action_null)),
         (TrailingWS, Box::new(|_| true), End, build_error_action("WHITESPACE EXPECTED")),

         (OpRelComp, Box::new(|c| c == '='), TrailingWS, build_action(Cat::OpRel, false)),
         (OpRelComp, Box::new(|c| c.is_whitespace()), TrailingWS, Box::new(action_lambda)),
         (OpRelComp, Box::new(|_| true), Error, build_error_action("WHITESPACE OR = EXPECTED")),

         (Id, Box::new(|c| c.is_alphabetic()), Id, Box::new(action_id)),
         (Id, Box::new(|c| c.is_whitespace()), TrailingWS, Box::new(action_id_try_reserved)),
         (Id, Box::new(|c| c == ')'), End, Box::new(action_id_try_reserved)),
         (Id, Box::new(|_| true), Error, build_error_action("BAD Id")),

         (Number, Box::new(|c| c.is_numeric()), Number, build_action(Cat::Number, false)),
         (Number, Box::new(|c| c.is_whitespace()), TrailingWS, Box::new(action_lambda)),
         (Number, Box::new(|c| c == ')'), End, Box::new(action_lambda)),
         (Number, Box::new(|_| true), Error, build_error_action("BAD Number")),

         (Str, Box::new(|c| c == '"'), StrEnd, build_action(Cat::Str, false)),
         (Str, Box::new(|_| true), Str, build_action(Cat::Str, false)),

         (StrEnd, Box::new(|c| c.is_whitespace()), TrailingWS, Box::new(action_lambda)),
         (StrEnd, Box::new(|c| c == ')'), End, Box::new(action_lambda)),
         (StrEnd, Box::new(|_| true), Error, build_error_action("BAD Str"))]
}
